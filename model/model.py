...
class InteractionDecoder(torch.nn.Module):
    def __init__(self, latent_size=128,mlp_layers=2):
        ...
        self.lambda_decoder      = build_mlp_d(latent_size, latent_size, 1, num_layers=mlp_layers, lay_norm=False)
    ...
    def forward(self, edge_index, senders_pos, receivers_pos, vector_a, vector_b, vector_c, interaction_latent, node_latent):
        ...
        lambda_edge   = self.lambda_decoder(interaction_latent)
        ...
        # Compute torque
        dsij = dlij - torch.cross(receivers_pos - r0ij, dpij*lambda_edge, dim=1)    
...
class DynamicsSolver(torch.nn.Module):
    ...
    def forward(self, graph):
        ...
        # Get or initialize angular quantities
