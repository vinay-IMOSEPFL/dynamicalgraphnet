...
class RefFrameCalc(nn.Module):
    ...
    def forward(
        ...
        senders_acc, receivers_acc,
        senders_alpha, receivers_alpha,
        ):
        ...
        diff_acc = receivers_acc - senders_acc
        sum_acc = senders_acc + receivers_acc
        
        diff_alpha = receivers_alpha - senders_alpha
        sum_alpha = senders_alpha + receivers_alpha
        ...
        b_b = normalize(sum_vel)
        b_c = normalize(torch.cross(diff_omega, vector_a, dim=1))
        b_d = normalize(sum_omega)
        b_e = normalize(torch.cross(diff_acc, vector_a, dim=1))
        b_f = normalize(sum_acc)
        b_g = normalize(torch.cross(diff_alpha, vector_a, dim=1))
        b_h = normalize(sum_alpha)

        # Combine to form b
        b = b_a + b_b + b_c + b_d +  b_e + b_f + b_g + b_h
    ...
    def forward(self, graph):
        ...
        for i in range(self.num_messages):
            ...
            vec_a, vec_b, vec_c = self.refframecalc(
                ...
                s_vt_, r_vt_, s_wt, r_wt,s_at,s_alphat
            )
